<template>
    <transition name="fade">
        <div v-if="oauthUser.active" class="backdrop delete-backdrop" @click.prevent="this.$emit('closeModal')"></div>
    </transition>
    <transition name="slide">
        <div v-if="oauthUser.active" class="floating-container">
            <div class="floating-wrap flex-col gap-16">
                <a href="#" class="alert-close flex justify-content-center align-items-center" @click.prevent="this.$emit('closeModal')">
                ...
                <div class="flex-col gap-8">
                    ...
                    <h2 class="mg-0">Do you wish to Sign Up to Flexsale with {{ oauthUser.type }}?</h2>
                </div>
                <div>
                    <svg v-if="oauthUser.type == 'google'" xmlns="http://www.w3.org/2000/svg" height="33" viewBox="0 0 31.341 32.021">
                    ...
                    <svg v-else-if="oauthUser.type == 'facebook'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="33" viewBox="0 0 32.021 31.86">
                        <defs>
                ...
                <div class="flex align-items-center w-100 gap-8 oauth-user">
                    <div class="oauth-user-img-wrap br-50"><img :src="oauthUser.user.picture" :alt="oauthUser.user.given_name"></div>
                    <div class="flex-col osr-info-hold">
                        <div class="oauth-user-name text-overflow-ellipsis">{{ oauthUser.user.name }}</div>
                        <div class="oauth-user-email text-overflow-ellipsis">{{ oauthUser.user.email }}</div>
                    </div>
                </div>
                <button class="button button-primary logon-btn" @click.prevent=""  :class="{ 'button-disabled' : creating }" :disabled="creating? true : false">
                    <span>{{ creating ? 'Creating account' : 'Sign Up with '+ oauthUser.user.given_name }}</span>
