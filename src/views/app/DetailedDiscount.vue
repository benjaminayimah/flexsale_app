        ...
        <div class="mini-body">
            <div class="mini-body-hold">
                <div class="mini-header-hold flex-row-js">
                    <h1 class="text-overflow-ellipsis">{{getThisDiscount.name}} </h1>
                    <div class="flex-row">
                        <button class="icon-label-btn flex-row" @click.prevent="$store.commit('getMainHomeWidth', 'discount')">
                        ...
                        <button class="icon-label-btn flex-row" @click.prevent="deleteDiscount($route.params.id)">
...
export default {
  components: { SelectedTagRow },
    ...
    computed: mapGetters(['getCheckedProducts', 'getThisDiscount', 'getToken', 'getHostname']),
    data() {
        return {
    ...
    methods: {
        ...
        deleteDiscount(id) {
            axios.delete(this.getHostname+'/api/discount/'+id+'?token='+this.getToken)
            .then((res) => {
                console.log(res.data)
                this.$store.commit('removeDeletedDiscount', res.data.id)
                const payload = {
                    id: 'success',
